        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          <div className="md:col-span-2">
            <div className="bg-white rounded-xl p-3 inline-block mb-6">
              <Image src="/logo-consisa.jpg" alt="CONSISA" width={140} height={52} className="h-12 w-auto object-contain" />
            </div>
            <p className="text-white/60 text-sm leading-relaxed mb-4 max-w-xs">
              Consultoría y Sistemas S.A.S. — Transformando organizaciones colombianas con Odoo, SAP e Inteligencia Artificial desde 2008.
            </p>
          ...
          {Object.entries(footerLinks).map(([section, links]) => (
            <div key={section}>
              <h4 className="font-bold text-white mb-5 text-sm uppercase tracking-wider">{section}</h4>
              <ul className="space-y-3">
                {links.map(link => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-white/60 text-sm hover:text-white transition-colors duration-200">{link.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
